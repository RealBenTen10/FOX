...
def extract_information_from_filename(filename):
    ...
    dataset_name = "_".join(name_parts[2:name_parts.index('one')])
    ...
    return learning_rate, batch_size, dataset_name, member_function, loss_function
...
def combine_csv_to_dataframe(folder_path):
    ...
    for file in os.listdir(folder_path):
        if file.endswith('.csv'):
            ...
            learning_rate, batch_size, dataset_name, member_function, loss_function = extract_information_from_filename(file)
...
folder_path = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'results_BATCH_LR'))
...
combined_results.to_csv('./combined_results_lr_batch.csv', index=False)
agg_combined_results.to_csv('./agg_combined_results_lr_batch.csv', index=False)

print("Kombinierte Datei wurde gespeichert als 'combined_results_lr_batch.csv'")
print("Aggregierte Datei wurde gespeichert als 'agg_combined_results_lr_batch.csv'")
