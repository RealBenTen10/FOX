...
def read_rule(model):
    ...
    for varname, members in model.layer['fuzzify'].varmfs.items():
        ...
        for mfname, mfdef in members.mfdefs.items():
            r.append(varname + ' _ {}: {}({})'.format(mfname, mfdef.__class__.__name__, ', '.join(
                ['{}={}'.format(n, p.item()) for n, p in mfdef.named_parameters()])))
        # r.append(']')
    r = np.array(r)
...
def metrics(dataset_name, columns_sel):
    ...
    while i < max_len:
        ...
        if len(target_test) == 1:
            ...
            print(
                str(index_len) + ";" + str(len(target_test)) + ";" + str(roc_auc_score(target_test, pred[:, 1])) + "\n")
